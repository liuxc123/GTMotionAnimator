//
//  CABasicAnimation+MotionAnimator.m
//  FBSnapshotTestCase
//
...
static BOOL IsAnimationKeyPathAlwaysNonAdditive(NSString *keyPath) {
    ...
    dispatch_once(&onceToken, ^{
        nonAdditiveKeyPaths = [NSSet setWithArray:@[GTMKeyPathAnchorPoint,
...
CABasicAnimation *GTMAnimationFromTraits(GTMAnimationTraits *traits, CGFloat timeScaleFactor) {
    ...
    CABasicAnimation *(^animationFromSpring)(GTMSpringTimingCurve *) =
    ^(GTMSpringTimingCurve *springTiming) {
...
#pragma clang diagnostic pop
    ...
    if ([traits.timingCurve isKindOfClass:[GTMSpringTimingCurveGenerator class]]) {
        GTMSpringTimingCurveGenerator *springTimingGenerator =
        (GTMSpringTimingCurveGenerator *)traits.timingCurve;
        return animationFromSpring(springTimingGenerator.springTimingCurve);
    }

    if ([traits.timingCurve isKindOfClass:[GTMSpringTimingCurve class]]) {
        return animationFromSpring((GTMSpringTimingCurve *)traits.timingCurve);
    }
...
BOOL GTMCanAnimationBeAdditive(NSString *keyPath, id toValue) {
...
void GTMConfigureAnimation(CABasicAnimation *animation, GTMAnimationTraits * traits) {
#pragma clang diagnostic push
    // CASpringAnimation is a private API on iOS 8 - we're able to make use of it because we're
    // linking against the public API on iOS 9+.
#pragma clang diagnostic ignored "-Wpartial-availability"
    BOOL isSpringAnimation = ([animation isKindOfClass:[CASpringAnimation class]]
                              && [traits.timingCurve isKindOfClass:[GTMSpringTimingCurve class]]
                              && [animation respondsToSelector:@selector(setInitialVelocity:)]);
    GTMSpringTimingCurve *springTimingCurve = (GTMSpringTimingCurve *)traits.timingCurve;
